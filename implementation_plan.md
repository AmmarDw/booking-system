...
- [x] **[Claude]** Frontend: booking now redirects away instead of showing an in-place toast — the confirmation toast (same §B.5 text) travels across the redirect via query params, then the URL is cleared via `router.replace` so a refresh doesn't re-show it. New `LoadingOverlay` component (full-page blur + spinner, content stays visible underneath, never disappears) shown for the duration of the booking request — covers the real latency of the synchronous Google Calendar + email round-trip server-side. Dashboard's consumer column bolds+colors the name and appends "(You)" when `consumerId` matches the signed-in user.
- *Acceptance:* ✅ curl-verified self-booking rejection (400) and that booking another provider's service as a provider still succeeds and appears in both the merged dashboard (marked via `consumerId`) and `?mine=true`. Browser-verified (chrome-devtools, network-throttled to actually see it): the loading overlay renders mid-request; a provider booking their own service's day view shows zero options ("No appointments for this date"); a provider's dashboard shows "Dr. Demo Provider (You)" in bold teal on their own cross-provider booking.

> **Follow-up correction (same day):** first pass opened `/appointments` and its nav link to every role, with booking always redirecting there. User pushed back — they want `/appointments` **consumer-only**, since the dashboard's "(You)" merge is the intended way for providers/admins to see their own consumer-side bookings, not a second page. Reverted: `/appointments` gate + nav link are consumer-only again (direct navigation by a provider/admin redirects to `/dashboard`); the post-booking redirect now branches by role (consumer → `/appointments`, provider/admin → `/dashboard`); `/dashboard` gained the same query-param-carried confirmation toast handling `/appointments` already had, so providers/admins don't lose that feedback.

### M6.3 — Claude-Design fidelity pass + status lifecycle + real dashboard analytics ✅ done, browser-tested (6 committed phases)
Grounded in the actual Claude Design source (fetched via DesignSync, not guessed). Each phase committed/pushed on its own.
- [x] **Phase 1 — Landing/Services fidelity:** landing "how it works" icon-in-circle + numbered-badge steps; service category icons; `/book` frontend search + duration-clock + provider-count icons (new `providerCount` on `GET /api/services`).
- [x] **Phase 2 — Booking calendar (`/book/[serviceId]`):** horizon note, availability legend, `Card`-wrapped calendar with chevron nav + year/month picker popover, responsive two-column (calendar + providers, stacks on mobile), multiple providers expandable at once, distinct current-day highlight, and a new **`full`** day bucket (total>0 but 0 available → keeps its "0/N" badge and stays clickable, no longer conflated with "nothing scheduled"). `NavBar` wraps + hides email on narrow viewports.
- [x] **Phase 3 — Bulk availability generator rebuild:** card-sectioned config, searchable admin provider grid, per-weekday time ranges, sticky live **preview** (slot count + mini calendar with scheduled/conflict coloring) via new non-destructive `POST /api/availability/bulk/preview`; `booking-window` now exposes `advanceLimitMonths`/`maxAdvanceDate`. Generation stays non-destructive (existing/booked slots skipped, never overwritten — flagged deviation from the design's "replace" copy).
- [x] **Phase 4 — Status lifecycle backend:** `BookingStatus` gains `COMPLETED`/`NO_SHOW`/`VACANT` (V7 migration; VACANT excluded from the DB constraint). `PATCH /api/bookings/{id}/status` enforces role + time gates + terminal-state finality (see CLAUDE.md §B.5.1). curl matrix fully verified (cancel ≥24h frees slot, <24h → 400; complete/no-show only after end; provider can't cancel → 403; re-transition → 409; ownership → 403).
- [x] **Phase 5 — Status lifecycle frontend:** shared `AppointmentDetailsModal` hosts role/time-gated action buttons (consumer cancel/no-show; provider/admin complete/no-show), shadowing disallowed ones with notes; all five statuses render with proper badges on both `/appointments` and `/dashboard`.
- [x] **Phase 6 — Shared dashboard rebuild:** `GET /api/bookings/{feed,stats,chart}` (multi-value CSV filters + date range); frontend `MultiSelect`; filter bar + 4 range-driven stat cards + inline SVG "bookings over time" chart + Calendar (42-cell month / week / day) + List, all reacting to shared filters + a Month/Week/Day granularity; synthesized `VACANT` open-slot entries surfaced only when opted into. (Design decisions confirmed with the user up front: cancel frees slot / no-show doesn't; complete/no-show gate on slot **end**; stat cards fully date-range-driven; 42-cell calendar-aligned month grid.)
